...
import { Upload, FileSpreadsheet, RefreshCcw, Filter, Settings, Edit2, Trash2, X, Plus, Calendar as CalendarIcon, Save, CreditCard, Banknote } from 'lucide-react'
import { ExpenseRecord, ExpenseCategory, CardSetting } from '@/lib/expense/types'
import { supabase } from '@/lib/supabaseClient'

// ... (Card, ManualAddModal, CategoryCell, EditModal, RowSetting 컴포넌트는 기존과 동일. 생략) ...
// (아래 코드는 ExpenseTrackerPage 메인 컴포넌트를 위주로 수정되었습니다.)

// --- (생략된 컴포넌트들: Card, ManualAddModal, CategoryCell, EditModal, RowSetting) ---
// 실제 파일에는 이전에 드린 컴포넌트 코드가 모두 있어야 합니다.
...
// ... ManualAddModal, CategoryCell, EditModal, RowSetting ... 
// (코드 길이가 길어 중복 생략합니다. 기존 코드 그대로 쓰시고 메인 컴포넌트만 아래 내용으로 교체하세요.)

/* 주의: ManualAddModal, CategoryCell, EditModal, RowSetting 컴포넌트 코드는 
   이전 답변의 코드를 그대로 사용해주세요.
   여기서는 메인 Page 컴포넌트의 변경점(필터 추가)만 반영하여 전체를 구성합니다.
*/

// (편의를 위해 전체 코드를 다시 드립니다. 이전 코드를 지우고 이걸 붙여넣으세요.)
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl w-full max-w-md p-6 space-y-4 shadow-xl">
        <h3 className="text-lg font-bold flex justify-between items-center">지출 내역 직접 추가 <button onClick={onClose}><X className="w-5 h-5"/></button></h3>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div><label className="text-xs font-bold text-gray-500 block mb-1">이용일</label><input type="date" name="transaction_date" value={formData.transaction_date} onChange={handleChange} className="border p-2 rounded w-full text-sm"/></div>
            <div><label className="text-xs font-bold text-gray-500 block mb-1">결제예정일(선택)</label><input type="date" name="payment_date" value={formData.payment_date} onChange={handleChange} placeholder="자동 계산됨" className="border p-2 rounded w-full text-sm"/></div>
          </div>
          <div><label className="text-xs font-bold text-gray-500 block mb-1">내역 (적요)</label><input type="text" name="description" value={formData.description} onChange={handleChange} placeholder="예: 스타벅스 커피" className="border p-2 rounded w-full text-sm"/></div>
          <div className="grid grid-cols-2 gap-2">
            <div><label className="text-xs font-bold text-gray-500 block mb-1">금액</label><input type="number" name="amount" value={formData.amount} onChange={handleChange} className="border p-2 rounded w-full text-sm font-mono"/></div>
            <div><label className="text-xs font-bold text-gray-500 block mb-1">결제수단</label>
               {settings.length > 0 ? ( <select name="card_company" value={formData.card_company} onChange={handleChange} className="border p-2 rounded w-full text-sm h-[38px]"> {settings.map(s => ( <option key={s.id} value={s.card_company}>{s.card_company}</option> ))} </select> ) : ( <input type="text" name="card_company" value={formData.card_company} onChange={handleChange} className="border p-2 rounded w-full text-sm" placeholder="직접 입력"/> )}
            </div>
          </div>
          <div><label className="text-xs font-bold text-gray-500 block mb-1">카테고리</label>
            <div className="flex gap-2">
                <select value={selectedParent} onChange={(e) => setSelectedParent(e.target.value)} className="border p-2 rounded w-1/2 text-sm">{parentCategories.map(p => <option key={p} value={p}>{p}</option>)}</select>
                <select value={formData.category_id || ''} onChange={(e) => setFormData(prev => ({...prev, category_id: Number(e.target.value)}))} className="border p-2 rounded w-1/2 text-sm">{subCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
            </div>
          </div>
        </div>
        <div className="flex justify-end gap-2 mt-4"><button onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded">취소</button><button onClick={() => onSave(formData)} className="px-4 py-2 bg-black text-white rounded hover:bg-gray-800">등록</button></div>
      </div>
    </div>
  )
}
...
function EditModal({ item, categories, onClose, onSave }: { item: ExpenseRecord; categories: ExpenseCategory[]; onClose: () => void; onSave: (updated: ExpenseRecord) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl w-full max-w-md p-6 space-y-4 shadow-xl">
        <h3 className="text-lg font-bold flex justify-between">내역 상세 수정 <button onClick={onClose}><X className="w-5 h-5"/></button></h3>
        <div className="space-y-3">
          <div><label className="text-xs text-gray-500 font-bold block mb-1">이용일 / 결제예정일</label><div className="flex gap-2"><input type="date" name="transaction_date" value={formData.transaction_date} onChange={handleChange} className="border p-2 rounded w-full text-sm"/><input type="date" name="payment_date" value={formData.payment_date || ''} onChange={handleChange} className="border p-2 rounded w-full text-sm"/></div></div>
          <div><label className="text-xs text-gray-500 font-bold block mb-1">적요 / 카드사</label><div className="flex gap-2"><input type="text" name="description" value={formData.description} onChange={handleChange} className="border p-2 rounded w-full text-sm"/><input type="text" name="card_company" value={formData.card_company} onChange={handleChange} className="border p-2 rounded w-1/3 text-sm"/></div></div>
          <div><label className="text-xs text-gray-500 font-bold block mb-1">금액</label><input type="number" name="amount" value={formData.amount} onChange={handleChange} className="border p-2 rounded w-full text-sm font-mono"/></div>
          <div><label className="text-xs text-gray-500 font-bold block mb-1">카테고리</label><div className="flex gap-2"><select value={selectedParent} onChange={(e) => { setSelectedParent(e.target.value); const first = categories.find(c => c.parent_name === e.target.value); if(first) setFormData(prev => ({...prev, category_id: first.id, category: first})); }} className="border p-2 rounded w-1/2 text-sm">{parentCategories.map(p => <option key={p} value={p}>{p}</option>)}</select><select name="category_id" value={formData.category_id || ''} onChange={(e) => { const catId = Number(e.target.value); const cat = categories.find(c => c.id === catId); setFormData(prev => ({ ...prev, category_id: catId, category: cat })); }} className="border p-2 rounded w-1/2 text-sm">{subCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div></div>
        </div>
        <div className="flex justify-end gap-2 mt-4"><button onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded">취소</button><button onClick={() => onSave(formData)} className="px-4 py-2 bg-black text-white rounded hover:bg-gray-800">저장</button></div>
      </div>
    </div>
  )
}
...
function RowSetting({ setting, onSave, onDelete }: { setting: CardSetting, onSave: (c: string, p: number, s: number, type: 'sliding' | 'immediate') => void, onDelete: (c: string) => void }) {
    ...
    return (
        <tr className="group">
            <td className="px-4 py-3 font-medium text-gray-700 flex items-center gap-2">{calcType === 'immediate' ? <Banknote className="w-4 h-4 text-green-600"/> : <CreditCard className="w-4 h-4 text-blue-600"/>}{setting.card_company}</td>
            <td className="px-4 py-3"><select value={calcType} onChange={(e) => setCalcType(e.target.value as any)} className="border rounded p-1 text-xs"><option value="sliding">신용카드</option><option value="immediate">현금/즉시</option></select></td>
            <td className="px-4 py-3">{calcType === 'sliding' ? (<div className="flex items-center gap-1"><span className="text-gray-400 text-xs">매월</span><input type="number" min="1" max="31" value={paymentDay} onChange={e => setPaymentDay(Number(e.target.value))} className="border rounded p-1 w-10 text-center font-mono text-xs"/><span className="text-gray-600 text-xs">일</span></div>) : <span className="text-gray-300 text-xs">-</span>}</td>
            <td className="px-4 py-3">{calcType === 'sliding' ? (<div className="flex items-center gap-1"><span className="text-gray-400 text-xs">전월</span><input type="number" min="1" max="31" value={startDay} onChange={e => setStartDay(Number(e.target.value))} className="border rounded p-1 w-10 text-center font-mono text-xs"/><span className="text-gray-600 text-xs">일부터</span></div>) : <span className="text-gray-300 text-xs">-</span>}</td>
            <td className="px-4 py-3 flex items-center gap-2"><button onClick={() => onSave(setting.card_company, paymentDay, startDay, calcType)} className="px-3 py-1.5 bg-black text-white text-xs rounded hover:bg-gray-800">수정</button><button onClick={() => onDelete(setting.card_company)} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4"/></button></td>
        </tr>
    )
}
...
export default function ExpenseTrackerPage() {
  ...
  const [filterCategory, setFilterCategory] = useState('ALL') // ✅ 카테고리 필터 상태
  ...
  useEffect(() => { loadData() }, [filterMode, startDate, endDate, filterCard, filterCategory]) // ✅ filterCategory 추가
  ...
  // ... (handleFileChange, handleSaveEdit 등 기존 핸들러 함수들 동일) ...
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div><h1 className="text-2xl font-bold tracking-tight">Expense Tracker</h1><p className="text-sm text-gray-500">지출 내역을 관리하고 분석합니다.</p></div>
