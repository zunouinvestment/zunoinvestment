  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <motion.div 
        ...
        className="bg-white rounded-xl w-full max-w-md p-5 shadow-2xl relative"
      >
        <h3 className="text-lg font-bold flex items-center gap-2 mb-4">
            <LinkIcon className="w-5 h-5"/> 카드사 바로가기
        </h3>
        ...
        <div className="grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto">
            {links.map((link) => (
                <a 
                    ...
                    className="flex items-center justify-between p-3 rounded-lg border hover:bg-gray-50 active:bg-gray-100 transition-colors"
                >
                    <span className={`text-sm font-bold ${link.color}`}>{link.name}</span>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <motion.div 
        ...
        className="bg-white rounded-xl w-full max-w-md p-5 space-y-4 shadow-xl max-h-[90vh] overflow-y-auto"
      >
        <h3 className="text-lg font-bold flex justify-between items-center">
          내역 추가 <button onClick={onClose}><X className="w-6 h-6 text-gray-500"/></button>
        </h3>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
               ...
               <input type="date" name="payment_date" value={formData.payment_date} onChange={handleChange} className="border p-2 rounded w-full text-sm text-gray-400"/>
            </div>
          </div>
          
          <div>
            <label className="text-xs font-bold text-gray-500 block mb-1">내역</label>
            <input type="text" name="description" value={formData.description} onChange={handleChange} placeholder="내용 입력" className="border p-2 rounded w-full text-sm"/>
          </div>
            ...
            <div>
               ...
               {settings.length > 0 ? (
                   <select 
                       ...
                       className="border p-2 rounded w-full text-sm h-[38px] bg-white"
                   >
                       {settings.map(s => (
        ...
        <div className="flex gap-2 mt-4 pt-2">
          <button onClick={onClose} className="flex-1 py-3 text-gray-600 bg-gray-100 rounded-lg text-sm font-medium">취소</button>
          <button onClick={() => onSave(formData)} className="flex-1 py-3 bg-black text-white rounded-lg text-sm font-bold">등록</button>
        </div>
      </motion.div>
    </div>
  )
...
function CategoryCell({ item, categories, onUpdate }: { item: ExpenseRecord; categories: ExpenseCategory[]; onUpdate: (id: number, catId: number) => void }) {
    ...
    if (isEditing) {
        return (
            <div className="flex flex-col gap-1 min-w-[120px] z-20 relative bg-white p-1 border rounded shadow-xl">
        ...
        <div onClick={startEdit} className="cursor-pointer group relative">
            {item.category ? (
                <div className="flex flex-col items-start gap-0.5">
                    <span className="text-[10px] text-gray-500 font-semibold flex items-center gap-1 whitespace-nowrap">
                        {item.category.parent_name}
                        <Edit2 className="w-2 h-2 opacity-0 group-hover:opacity-100 transition-opacity text-gray-400" />
                    </span>
                    <span className="inline-flex px-1.5 py-0.5 rounded text-[10px] md:text-[11px] font-medium text-white shadow-sm hover:opacity-80 transition-opacity whitespace-nowrap" style={{ backgroundColor: item.category.color }}>
                        {item.category.name}
                    </span>
                </div>
            ) : <span className="text-gray-300 text-xs hover:text-gray-500 whitespace-nowrap">카테고리 없음</span>}
        </div>
    )
}
      ...
      >
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label className="text-xs font-bold text-gray-500 block mb-1">결제예정일</label><input type="date" name="payment_date" value={formData.payment_date || ''} onChange={handleChange} className="border p-2 rounded w-full text-sm"/></div>
          </div>
          <div>
            <label className="text-xs font-bold text-gray-500 block mb-1">내역</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label className="text-xs font-bold text-gray-500 block mb-1">카드사</label><input type="text" name="card_company" value={formData.card_company} onChange={handleChange} className="border p-2 rounded w-full text-sm"/></div>
          </div>
          <div>
            <label className="text-xs font-bold text-gray-500 block mb-1">카테고리</label>
            <div className="flex gap-2">
                <select value={selectedParent} onChange={(e) => { setSelectedParent(e.target.value); const first = categories.find(c => c.parent_name === e.target.value); if(first) setFormData(prev => ({...prev, category_id: first.id, category: first})); }} className="border p-2 rounded w-1/2 text-sm bg-white">
                    {parentCategories.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
                <select name="category_id" value={formData.category_id || ''} onChange={(e) => { const catId = Number(e.target.value); const cat = categories.find(c => c.id === catId); setFormData(prev => ({ ...prev, category_id: catId, category: cat })); }} className="border p-2 rounded w-1/2 text-sm bg-white">
                    {subCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
            </div>
          </div>
        ...
        <div className="flex gap-2 mt-4 pt-2">
            <button onClick={onClose} className="flex-1 py-3 text-gray-600 bg-gray-100 rounded-lg text-sm font-medium">취소</button>
            <button onClick={() => onSave(formData)} className="flex-1 py-3 bg-black text-white rounded-lg text-sm font-bold">저장</button>
        </div>
      </motion.div>
    </div>
  )
...
function RowSetting({ setting, onSave, onDelete }: { setting: CardSetting, onSave: (c: string, p: number, s: number, type: 'sliding' | 'immediate') => void, onDelete: (c: string) => void }) {
    ...
    return (
        <tr className="group hover:bg-gray-50 transition-colors">
            <td className="px-3 py-3 md:px-4 font-medium text-gray-700 flex items-center gap-2 text-xs md:text-sm whitespace-nowrap">
                {calcType === 'immediate' ? <Banknote className="w-4 h-4 text-green-600"/> : <CreditCard className="w-4 h-4 text-blue-600"/>}{setting.card_company}
            </td>
            ...
            <td className="px-3 py-3 md:px-4 flex items-center gap-1">
                <button onClick={() => onSave(setting.card_company, paymentDay, startDay, calcType)} className="px-2 py-1 bg-black text-white text-xs rounded hover:bg-gray-800">수정</button>
                <button onClick={() => onDelete(setting.card_company)} className="p-1 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4"/></button>
            </td>
        </tr>
    )
}
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold tracking-tight">Expense Tracker</h1>
          <p className="text-xs md:text-sm text-gray-500">지출 내역을 관리하고 분석합니다.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
            ...
            <button 
                ...
                className="flex items-center gap-1 md:gap-2 bg-white border border-gray-300 text-gray-700 px-2 py-1.5 md:px-3 md:py-2 rounded-md hover:bg-gray-50 transition text-xs md:text-sm font-medium whitespace-nowrap"
            >
                <LinkIcon className="w-3 h-3 md:w-4 md:h-4" /> 카드사
            </button>
      ...
      {viewMode === 'list' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
            ...
            <div className="bg-white p-3 md:p-4 rounded-xl border flex flex-wrap gap-2 md:gap-4 items-center shadow-sm text-xs md:text-sm">
                ...
                <div className="flex items-center gap-1 md:gap-2 ml-auto w-1/2 md:w-auto">
                    <select className="border rounded p-1 outline-none focus:border-black w-full md:w-[120px]" value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}>
                        <option value="ALL">전체 카테고리</option>
                        {parentCategories.map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                </div>

                <div className="flex items-center gap-1 md:gap-2 w-[45%] md:w-auto">
                    <select className="border rounded p-1 outline-none focus:border-black w-full md:w-[120px]" value={filterCard} onChange={(e) => setFilterCard(e.target.value)}>
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            ...
                            : ( expenses.map((item) => (
                                    <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                                        <td className="px-3 py-2 md:px-6 md:py-3 text-gray-600 font-mono whitespace-nowrap">
                                            {item.transaction_date.slice(5)} {/* 월-일만 표시 */}
                                            <div className="text-[9px] text-gray-400 md:hidden">{item.card_company}</div>
                                        </td>
                                        ...
                                        <td className="px-3 py-2 md:px-6 md:py-3 flex justify-center gap-1 md:gap-2">
                                            <button onClick={() => setEditingItem(item)} className="p-1 hover:bg-gray-200 rounded text-blue-600"><Edit2 className="w-3 h-3 md:w-4 md:h-4"/></button>
                                            <button onClick={() => handleDelete(item.id)} className="p-1 hover:bg-gray-200 rounded text-red-600"><Trash2 className="w-3 h-3 md:w-4 md:h-4"/></button>
                                        </td>
                                    </tr>
                            )))}
                        </tbody>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="md:col-span-2 overflow-hidden">
                    <h2 className="text-lg font-bold mb-4 flex items-center gap-2"><Settings className="w-5 h-5"/> 결제 설정</h2>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-gray-50 text-gray-500 border-b whitespace-nowrap">
                                <tr><th className="px-3 py-2 md:px-4">명칭</th><th className="px-3 py-2 md:px-4">유형</th><th className="px-3 py-2 md:px-4">결제일</th><th className="px-3 py-2 md:px-4">시작일</th><th className="px-3 py-2 md:px-4">관리</th></tr>
                            </thead>
                            <tbody className="divide-y">{cardSettings.map(setting => <RowSetting key={setting.card_company} setting={setting} onSave={handleSaveSetting} onDelete={handleDeleteSetting} />)}</tbody>
                        </table>
                    </div>
                </Card>
