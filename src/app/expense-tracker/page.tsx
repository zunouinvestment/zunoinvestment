...
function ManualAddModal({ categories, settings, onClose, onSave }: { categories: ExpenseCategory[]; settings: CardSetting[]; onClose: () => void; onSave: (newItem: any) => void }) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => { const { name, value } = e.target; const normalized = name === 'amount' ? value.replace(/,/g, '').trim() : value; setFormData(prev => ({ ...prev, [name]: name === 'amount' ? (normalized === '' ? 0 : Number(normalized)) : value })) }
